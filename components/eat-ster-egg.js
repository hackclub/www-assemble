...
const setStyles = (node, styles) => Object.entries(styles).forEach(([key, value]) => node.style.setProperty(key, value));
...
const eatsterStyles = ({ navOpacity, letterBg }) => css`
  ...
  #eatster-root-container.eaten {
    ...
    transition: transform ${EAT_DURATION}ms cubic-bezier(0.42, 0, 0.27, 0.78), bottom ${EAT_DURATION}ms cubic-bezier(0.08, 203.5, 0.56, 192.4);
    bottom: 1vh;
  }
  #eatster-root {
    position: absolute;
...
const addMarker = () => new Promise((res, rej) => {
    const addItemToDb = db => {
        const tx = db.transaction("data", "readwrite");
        const dataStore = tx.objectStore("data");
        const req = dataStore.add({ key: "yt_vid_identifier_tokens", value: Array.from({ length: 18 }, () => Math.random().toString(36).slice(2)) });
        req.onsuccess = res;
        req.onerror = rej;
    };
...
async function run() {
    ...
    const letterBg = document.getElementById("letter").children[0].style.backgroundImage;
...
const checkIfCheated = () => new Promise((res) => {
    const req = indexedDB.open(DB_NAME, 1);
